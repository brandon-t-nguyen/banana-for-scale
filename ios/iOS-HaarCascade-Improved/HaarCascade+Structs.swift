...
extension HaarCascade {
    ...
    func getBuffersAndTexture() -> (Buffer<Float32>, Texture, Texture) {
        ...
        classifiersDescriptor.usage = [.shaderRead, .shaderWrite]
        var toPad = (16-classifierArray.count%16)
        if toPad > 0 {
            classifierArray = classifierArray + [Float32](repeating: 0.0, count: toPad)
        }
        ...
        // pad the rectsArray
        toPad = (16-rectsArray.count%16)
        if toPad > 0 {
            rectsArray = rectsArray + [UInt32](repeating: 0, count: toPad)
        }
        ...
        rectDescriptor.usage = [.shaderRead, .shaderWrite]
        let rectsArrayBytesPerRow = rectsArray.count * MemoryLayout<UInt32>.size
